...
const DetailedResults = ({ analysisResult }) => {
    ...
    const featureBreakdown = useMemo(() => {
        ...
        const featureOrder = [
            ...
            { key: 'd_height', name: 'D-Height' },
            { key: 'd_loop', name: 'D-Loop' },
            { key: 't_height', name: 'T-Height' },
            { key: 't_bar', name: 'T-Bar' },
            { key: 't_lean', name: 'T-Lean' },
            ...
            <div className="feature-breakdown-container card">
                ...
                <div className="feature-list">
                    {featureBreakdown.map((feature, index) => {
                        const isHighlighted = highImpactTraits[outcome]?.includes(feature.key);
                        ...
                        <button
                            onClick={async () => {
                                try {
                                    ...
                                    const data = await res.json();
                                    console.log('Feature Debug Data:', data);
                                    alert(`Found ${data.totalSamples} samples, ${data.samplesWithFeatures} have features. Check console for details.`);
                                } catch (err) {
                                    console.error('Error fetching feature debug data:', err);
                                }
                            }}
                            className="nav-button"
                            style={{ backgroundColor: '#8b5cf6', padding: '0.5rem 1rem', fontSize: '0.875rem' }}
                        >
                            Debug Features
                        </button>
                        <button
            ...
            <div className="dashboard-grid">
                ...
                <div className="card full-width">
                    ...
                    <p style={{ fontSize: '0.875rem', color: '#6b7280', marginBottom: '1rem' }}>
                        Download all analysis data including patient information, predictions, and questionnaire results in CSV format.
                    </p>
                    <button onClick={handleExport} className="nav-button" style={{ width: 'auto' }}>
                        Export Data as CSV
                    </button>
                </div>
            </div>
